...
class UpworkAutomationGraph:
    ...
    def need_to_process_matches(self, state):
        ...
        if len(state['matches']) == 0:
            ...
            print(Fore.GREEN + f"There are {len(state['matches'])} Job matches to process\n" + Style.RESET_ALL)
            return "Process jobs"
    ...
    def init_agents(self):
        ...
        self.classify_jobs_agent = Agent(
            ...
            system_prompt=classify_jobs_prompt.format(profile)
        )
        self.generate_cover_letter_agent = Agent(
            name="Writer Agent",
            model="groq/llama3-70b-8192", 
            system_prompt=generate_cover_letter_prompt.format(profile)
        )
    ...
    def build_graph(self):
        ...
        graph.add_conditional_edges(
            ...
            {
                "Process jobs": "generate_cover_letter",
                "No matches": END
            }
        )
